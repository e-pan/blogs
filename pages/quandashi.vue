<template>
  <div>
    <div class="postion">数据来源：权大师</div>
    <div class="wrap1">
      <div class="left">
        <div class="items">
          <!-- <h1>基础信息</h1> -->
          <div class="item">
            ...
            <img
              :src="dataInfo.imageInfo && dataInfo.imageInfo.original_logo"
            />
          </div>
          <div class="item">
            <span class="label">注册号：</span
            ><span class="content" id="regNo">{{
              dataInfo.baseInfo.dataId
            }}</span>
          </div>
          <div class="item">
          ...
          <div class="item">
            ...
            ><span class="content">{{
              dataInfo.baseInfo.applicantShare_1
            }}</span>
          </div>
          <div class="item">
            <span class="label">共有申请人2：</span
            ><span class="content">{{
              dataInfo.baseInfo.applicantShare_2
            }}</span>
          </div>
          <div class="item">
            ...
            ><span
              class="content"
              v-html="dataInfo.baseInfo.enApplicant"
            ></span>
          </div>
          <div class="item">
            <span class="label">申请人地址英文：</span
            ><span class="content" v-html="dataInfo.baseInfo.enAddress"></span>
          </div>
          <div class="item">
          ...
          <div class="item">
            ...
            ><span class="content">{{
              dataInfo.baseInfo.tmNameForm == "0"
                ? "文字"
                : dataInfo.baseInfo.tmNameForm == 1
                ? "图形"
                : "文字及图"
            }}</span>
          </div>
      ...
      <div class="right">
        <div class="items">
          ...
          <div
            class="item"
            v-if="dataInfo.noticeInfo && dataInfo.noticeInfo.length"
          >
            ...
            <div class="content goods-items">
              <p
                ...
                :key="index"
              >
                {{ item.noticeDate }} 第{{ item.issue }}期《{{
                  item.noticeName
                }}》 第{{ item.page }}页
                <a target="_blank" :href="item.imgUrl">查看</a>
              </p>
            </div>
          </div>
          <!-- <h1>使用商品</h1> -->
          <div
            class="item"
            v-if="dataInfo.goodsInfo && dataInfo.goodsInfo.length"
          >
            ...
            <div class="content goods-items">
              ...
              >
                {{ good.name }}({{ good.code }}
                <b v-if="good.status == '0'">正常</b>
                <span v-else>
                  {{ good.status == 1 ? "预删除" : "已删除" }} </span
                >)
              </div>
            </div>
          </div>
...
export default {
  async asyncData(context) {
    ...
    let dataInfo = await axiosAjax("/trademark/quandashi/query", {
      id: context.query.id,
    });

    ...
    <div class="wrap home">
      <div class="main">
        <section v-for="(item, index) in articles" :key="index" @click="articleDetail(item)">
          ...
          <img :src="item.thumbnail + '?x-oss-process=image/resize,m_fill,h_200,w_752'" v-if="item.thumbnail">
      ...
      <aside class="side">
        <article class="tag">
          <h3>分类标签</h3>
          <div>
            <a v-for="(tag, index) in tags" :key="index" @click="handleTag(tag)" :href='"/tag?id=" + tag.id'>{{ tag.name }}</a>
          </div>
        </article>
